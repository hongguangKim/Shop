...
public class ActGoodsResultPresenterImpl implements IGoodsResultContract.IGoodsResultPresenter {
    ...
    private void queryDatasFromServer(final String datas) {
        ...
        if(type == IFragHomeContract.GOODS_TYPE_QUERY_FUNC)
            query.addWhereEqualTo("goodsTypeId",data[0]);
        else if(type == IFragHomeContract.GOODS_NAME_QUERY_FUNC)
            query.addWhereEqualTo("goodsName",data[0]);
